...
async function scrape(url) {
  ...
  try {
    ...
    await page.waitForSelector('input[name="URLz"]', { timeout: 15000 });
    await page.type('input[name="URLz"]', url);
    await page.click('button[type="submit"]');
    await page.waitForSelector("#sdlink, .alert-danger", { timeout: 30000 });

    const results = await page.evaluate(() => {
      const errorEl = document.querySelector(".alert-danger");
      if (errorEl) {
        return { error: errorEl.innerText.trim() };
      }

      const titleEl = document.querySelector(".lib-text");
      const title = titleEl ? titleEl.innerText.trim() : "Facebook Video";

      const sdLinkEl = document.getElementById("sdlink");
      ...
      if (sdLinkEl) {
        downloads.push({
          quality: "Normal (SD)",
          url: sdLinkEl.getAttribute("href"),
        });
      }
      if (hdLinkEl) {
        downloads.push({ quality: "HD", url: hdLinkEl.getAttribute("href") });
      }
